...
const ALLOWED_AUTH_ORIGIN = "https://auth.openai.com";
function assertAllowedAuthUrl(input) {
    const url = new URL(typeof input === "string" ? input : input.url);
    if (url.origin !== ALLOWED_AUTH_ORIGIN) {
        throw new Error(`Refusing OAuth request to non-OpenAI origin: ${url.origin}`);
    }
}
...
async function refreshAccessToken(refreshToken, signal) {
    let response;
    try {
        response = await fetchWithLoginCancellation(TOKEN_URL, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({
