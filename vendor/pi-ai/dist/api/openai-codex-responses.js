...
const ALLOWED_CODEX_ORIGIN = "https://chatgpt.com";
...
function resolveCodexUrl(baseUrl) {
    ...
    const resolved = normalized.endsWith("/codex/responses")
        ? normalized
        : normalized.endsWith("/codex")
            ? `${normalized}/responses`
            : `${normalized}/codex/responses`;
    const url = new URL(resolved);
    if (url.origin !== ALLOWED_CODEX_ORIGIN || url.pathname !== "/backend-api/codex/responses") {
        throw new Error(`Refusing Codex request to unapproved endpoint: ${url.origin}${url.pathname}`);
    }
    return url.toString();
}
